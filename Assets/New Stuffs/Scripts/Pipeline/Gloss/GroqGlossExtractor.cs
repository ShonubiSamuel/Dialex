...
{
    /// <summary>
    /// Gloss extraction via Groq (OpenAI-compatible Chat Completions).
    /// Endpoint: https://api.groq.com/openai/v1/chat/completions
    /// Returns a JSON array or plain text lines; we parse robustly then post-process.
    /// </summary>
    public class GroqGlossExtractor : MonoBehaviour,
        IGlossExtractor, SignPipelineController.IGlossExtractor
    {
            ...
            Action<Exception> onError = null)
        {
            ...
            {
                model = string.IsNullOrEmpty(prompt.model) ? "llama-3.1-70b-versatile" : prompt.model,
            ...
            if (logRequest) Debug.Log($"[GroqGlossExtractor] POST {endpointUrl}\n{json}");
            ...
            if (logResponse) Debug.Log($"[GroqGlossExtractor] Raw:\n{raw}");
        ...
        {
            // Try to deserialize as OpenAI-style response
            try
            {
            ...
            {
                Debug.LogWarning($"[GroqGlossExtractor] ParseChoices failed: {e.Message}");
            }
            // Fallback: try to parse the body directly (some gateways proxy the raw array)
